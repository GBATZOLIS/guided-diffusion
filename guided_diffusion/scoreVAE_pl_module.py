...
class ScoreVAE(pl.LightningModule):
    ...
    def encode(self, x):
        ...
        latent_distribution_parameters = self.encoder(x, torch.full(size=(x.size(0),), fill_value=-1).long().to(self.device)) #self.encoder(x, th.zeros(size=(x.size(0),)).to(self.device))
