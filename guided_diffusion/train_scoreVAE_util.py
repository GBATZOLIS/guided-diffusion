...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        save_interval,
        resume_checkpoint,
        diffusion_model_checkpoint,  #new
        use_fp16=False,
        ...
        self._load_n_freeze_pretrained_diffusion_model()
    ...
    def _load_n_freeze_pretrained_diffusion_model(self):
        if dist.get_rank() == 0:
            logger.log(f"loading pretrained score model from checkpoint: {self.diffusion_model_checkpoint}...")
            self.diffusion_model.load_state_dict(
                dist_util.load_state_dict(
                    self.diffusion_model_checkpoint, map_location=dist_util.dev()
                )
            )
        ...
        #we don't need synchronisation for the score model because it remains unchanged
